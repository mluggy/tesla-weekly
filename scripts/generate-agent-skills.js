...
const skills = [
  ...
  },
  {
    ...
    // Body is the LITERAL contents of public/auth.md (generated by
    // generate-auth.js, which runs earlier in build-all.js). Keeping
    // them byte-identical (modulo our YAML frontmatter prepended by
    // the writer below) ensures orank's agent-auth-discovery deep
    // check — which content-compares this SKILL.md against /auth.md —
    // sees a structural match. The previous hand-coded body drifted
    // (extra "## When to use" preamble, "## Register (RFC 7591)" vs
    // bare "## Register") and tripped the comparator.
    body: readFileSync("public/auth.md", "utf8").trim(),
  },
];
